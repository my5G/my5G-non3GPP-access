...
func (ue *UeRanContext) GetSecurityULCount() []byte {
	var r = make([]byte, 4)
	binary.BigEndian.PutUint32(r, ue.ULCount&0xffffff)
	return r
}

func (ue *UeRanContext) GetSecurityDLCount() []byte {
	var r = make([]byte, 4)
...
func (ue *UeRanContext) DeriveRESstarAndSetKey(authSubs models.AuthenticationSubscription, RAND []byte, snNmae string) []byte {
